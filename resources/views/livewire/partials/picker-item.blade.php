@if ($item['type'] === 'directory')
    <button type="button" class="ufm-picker-explorer__item" wire:click="open({{ \Illuminate\Support\Js::from($item['path']) }})">
        ...
        <span class="ufm-picker-explorer__meta">Folder</span>
    </button>
@else
    <button type="button" class="ufm-picker-explorer__item" wire:click="select({{ \Illuminate\Support\Js::from($item['path']) }})" @class(['is-selected' => $multiple && $this->isSelected($item['path'])]) @disabled($multiple && ! $this->isSelected($item['path']) && $this->selectionLimitReached()) aria-pressed="{{ $multiple && $this->isSelected($item['path']) ? 'true' : 'false' }}">
        @if ($multiple && $this->isSelected($item['path']))
            <span class="ufm-picker-explorer__selection-check" aria-label="Selected">
